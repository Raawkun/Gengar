...
class Modules(commands.Cog):
    ...
    hunted_type = ""
    server_whitelist = [1351851125330022430, 825813023716540426, 1028465180994191381]
    ...
    async def adamannpc(self, message):
        ...
        await message.channel.send(f"<@{sender.id}> - Trainer **Irida** ``961``is ready for the next battle.\n**Don't forget to set the right team!!!** <:493:1213076751559819294>")
    ...
    async def dailyreset(self):
        while True:
            ...
            embed.add_field(name="**Coins**",value=f"Total: {(row[1]+row[2]+row[3]+row[4]+row[5]+row[8]):,} {coin}\nFrom Catches: {row[1]:,} {coin}\nFrom Battles: {row[2]:,} {coin}\nFrom Market: {row[3]:,} {coin}\nFrom Releasing: {row[4]:,} {coin}\nFrom World Boss: {row[5]:,} {coin}\nFrom other sources: {row[8]:,} {coin}",inline=True)
        ...
        if message.guild.id in Modules.server_whitelist: #paralympic
            ...
            if "opened " in message.content and ":lootbox:" in message.content:
                try:
                    ...
                    print(f"Soke error: {message.jump_url} - \n{e}")
    ...
    async def typehunt(self, message, sender):
        ...
        if points > 1:
            sing = "points"
        appending = f"{type_emotes[ev[5]]} You earned {th_points[data[14]]} {sing} for your catch!\nYou now have {points} points!"
