...
For more information use:
    ...
    async def _starterselect(self, ctx, mon, client):
        ...
        if mon is None:
            ...
            self.db.commit()
            self.db.execute(f'INSERT INTO Johto (User_ID, Name, Pallet_Mon) VALUES ({(member.id)}, "{member.name}", {mon})')
            self.db.commit()
            self.db.execute(f'UPDATE User SET Johto_Mon = {mon} WHERE id = {member.id}')
            self.db.commit()
